  ...
  <article class="project-card" :class="[`project-card--${project.preview.kind}`, { 'project-card--featured': project.featured, 'is-active': active }]">
    ...
    <div class="project-card__frame">
      <div class="project-card__screen">
        <div class="project-card__browserbar" aria-hidden="true"><i></i><i></i><i></i><span>{{ project.title }}</span><b>AD-NET / LIVE</b></div>
...
fetch("imgflip.com/api/get_memes")
  .then(response =&gt; response.json())</code></pre></div>
      ...
      <div class="project-card__console">
        <span class="project-card__status" aria-hidden="true"><i></i> online</span>
        <div class="project-card__actions" :aria-label="`${project.title} actions`">
          <a v-if="project.liveUrl" :href="project.liveUrl" target="_blank" rel="noreferrer">Demo <span aria-hidden="true">↗</span></a>
          <a :href="project.repositoryUrl" target="_blank" rel="noreferrer">Code <span aria-hidden="true">↗</span></a>
          <button type="button" @click="$emit('details', project, $event.currentTarget as HTMLElement)">Info <span aria-hidden="true">＋</span></button>
        </div>
        <span class="project-card__unit" aria-hidden="true">UNIT {{ String(index + 1).padStart(2, "0") }}</span>
      </div>
    </div>
