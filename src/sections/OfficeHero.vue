...
import DailyChessPuzzle from "../components/DailyChessPuzzle.vue";
...
const sourceLabel = computed(
  () =>
    ({ live: "Live", cache: "Cached", staleCache: "Cached", fallback: "Unavailable", preview: "Preview" })[
      state.value.source
    ],
);
function collapseIntroduction() {
  if (showIntroduction.value) showIntroduction.value = false;
}
  ...
  <section id="office" class="hero floor-section" aria-labelledby="hero-title" @click="collapseIntroduction">
    ...
    <div class="hero-copy-drawer" :class="{ 'hero-copy-drawer--closed': !showIntroduction }">
      <div
        ...
        class="hero__content hero__plate"
        data-parallax="-2"
        ...
        <div class="hero__status" aria-label="Current Athens time and weather">
          <span
            ><b>Athens</b><time :datetime="state.lastUpdated.toISOString()">{{ state.localTime }}</time></span
          >
          <span
            ><b>Weather · {{ sourceLabel }}</b
            >{{ Math.round(state.temperature) }}°C · {{ weatherLabel }}</span
          >
        </div>
        <div class="actions">
          <a class="button button--primary" href="#projects">View selected work <span>↓</span></a
          ><a class="button button--ghost" href="#about">About the developer</a>
        </div>
      </div>
      <button
        class="hero-copy-toggle"
        type="button"
        :aria-expanded="showIntroduction"
        aria-controls="hero-introduction"
        @click.stop="toggleIntroduction">
        <span aria-hidden="true">{{ showIntroduction ? "‹" : "›" }}</span
        ><b>{{ showIntroduction ? "Hide intro" : "Show intro" }}</b>
      </button>
    </div>
